...
class GuardrailEngine:
    ...
    def check_budget(self, spent_so_far_gbp: float) -> GuardrailResult:
        ...
        pct = (
            (spent_so_far_gbp / self.budget_cap_gbp) * 100
            if self.budget_cap_gbp > 0
            else 0
        )
        if spent_so_far_gbp >= self.budget_cap_gbp:
            return GuardrailResult(
                ...
                value=spent_so_far_gbp,
                threshold=self.budget_cap_gbp,
            )
        if pct >= 80:
            return GuardrailResult(
                passed=True,
        ...
        return self._pass(
            "budget_cap", value=spent_so_far_gbp, threshold=self.budget_cap_gbp
        )
    ...
    def check_pii(self, pii_rows: int) -> GuardrailResult:
        ...
        if pii_rows > self.max_pii_rows:
            return GuardrailResult(
                ...
                guardrail_type="pii_rows_detected",
                value=pii_rows,
                threshold=self.max_pii_rows,
            )
        return self._pass(
            "pii_rows_detected", value=pii_rows, threshold=self.max_pii_rows
        )
    ...
    def check_completeness(self, score: float) -> GuardrailResult:
        ...
        if score < self.min_completeness:
            return GuardrailResult(
                ...
                guardrail_type="completeness_score",
                value=score,
                threshold=self.min_completeness,
            )
        return self._pass(
            "completeness_score", value=score, threshold=self.min_completeness
        )
    ...
    def check_anomaly_score(self, score: float) -> GuardrailResult:
        ...
        if score >= self.anomaly_score_warn:
            return GuardrailResult(
                ...
                guardrail_type="anomaly_score",
                value=score,
                threshold=self.anomaly_score_warn,
            )
        return self._pass(
            "anomaly_score", value=score, threshold=self.anomaly_score_warn
        )
    ...
    def check_parse_failures(self, failure_count: int) -> GuardrailResult:
        ...
        if failure_count >= self.max_parse_failures:
            return GuardrailResult(
                ...
                guardrail_type="parse_failure_streak",
                value=failure_count,
                threshold=self.max_parse_failures,
            )
        return self._pass(
    ...
    def _pass(
        self, guardrail_type: str, value: float = 0, threshold: float = 0
    ) -> GuardrailResult:
        return GuardrailResult(
            passed=True,
            action="continue",
