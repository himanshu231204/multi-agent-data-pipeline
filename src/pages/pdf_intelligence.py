...
def render(visitor_fp: str):
    ...
    # ── PDF MODE SELECTOR ────────────────────────────────────
    st.markdown(
        '<div class="section-label">Select Run Mode</div>', unsafe_allow_html=True
    )
    ...
    with col_pb:
        ...
        pb_tick = (
            '<div class="selected-tick tick-baseline">&#10003;</div>'
            if not pdf_routing_on
            else ""
        )
        html_pb = (
        ...
        if st.button(
            "Select: Without Router (Baseline)",
            key="pdf_sel_baseline",
    ...
    with col_pr:
        ...
        pr_tick = (
            '<div class="selected-tick tick-router">&#10003;</div>'
            if pdf_routing_on
            else ""
        )
        html_pr = (
        ...
        if st.button(
            "Select: With Router", key="pdf_sel_router", use_container_width=True
        ):
    ...
    # ── PDF AUTH & CREDITS ───────────────────────────────────
    st.markdown(
        '<div class="section-label">GitHub Access &amp; Free Credits</div>',
    ...
    # ── PDF UPLOAD ───────────────────────────────────────────
    st.markdown(
        '<div class="section-label">Upload a PDF document</div>', unsafe_allow_html=True
    )
    pdf_file = st.file_uploader(
        "Drop PDF here", type=["pdf"], label_visibility="collapsed"
    )
    use_demo_pdf = st.checkbox(
        "Use demo PDF — Project Alpha Q1 2024 quarterly review report"
    )
    ...
    if pdf_file:
        try:
            ...
            if st.button(
                f"▶ Run PDF Pipeline — {mode_label}",
                use_container_width=True,
                type="primary",
            ):
                run_ok, run_mode = check_can_run(
                    pdf_active_user, pdf_byok_key, visitor_fp
                )
                if not run_ok:
                    st.error(
                        "No free runs remaining. Enter your GitHub username above, add your API key, or fork the repo for lifetime access."
                    )
                    st.stop()
                ...
                tracer = RunTracer(
                    source="pdf", mode="router" if pdf_routing_on else "baseline"
                )

                with st.status(
                    f"Agent 1/5 — PDF Parser ({m_parser.split('-')[1].title()})",
                    expanded=False,
                ) as s:
                    sp1 = tracer.start_span("pdf_parser", m_parser)
                    parser_result = pdf_parser.run(
                        preview, total_pages, model=m_parser, api_key=_api, span=sp1
                    )
                    s.update(
                        label="✅ Agent 1/5 — PDF Parser complete", state="complete"
                    )
                progress.progress(0.2)

                with st.status(
                    f"Agent 2/5 — Entity Extractor ({m_entity.split('-')[1].title()})",
                    expanded=False,
                ) as s:
                    sp2 = tracer.start_span("entity_extractor", m_entity)
                    entity_result = entity_extractor.run(
                        preview, total_pages, model=m_entity, api_key=_api, span=sp2
                    )
                    s.update(
                        label="✅ Agent 2/5 — Entity Extractor complete",
                        state="complete",
                    )
                progress.progress(0.4)

                with st.status(
                    f"Agent 3/5 — Risk Detector ({m_risk.split('-')[1].title()})",
                    expanded=False,
                ) as s:
                    sp3 = tracer.start_span("risk_detector", m_risk)
                    risk_result = risk_detector.run(
                        preview, total_pages, model=m_risk, api_key=_api, span=sp3
                    )
                    s.update(
                        label="✅ Agent 3/5 — Risk Detector complete", state="complete"
                    )
                progress.progress(0.6)

                with st.status(
                    f"Agent 4/5 — Action Extractor ({m_action.split('-')[1].title()})",
                    expanded=False,
                ) as s:
                    sp4 = tracer.start_span("action_extractor", m_action)
                    action_result = action_extractor.run(
                        preview, total_pages, model=m_action, api_key=_api, span=sp4
                    )
                    s.update(
                        label="✅ Agent 4/5 — Action Extractor complete",
                        state="complete",
                    )
                ...
                with st.status(
                    f"Agent 5/5 — Summariser ({m_summary.split('-')[1].title()})",
                    expanded=False,
                ) as s:
                    sp5 = tracer.start_span("summariser", m_summary)
                    summariser_result = summariser.run(
                        preview, total_pages, ctx, model=m_summary, span=sp5
                    )
                    s.update(
                        label="✅ Agent 5/5 — Summariser complete", state="complete"
                    )
    ...
    if cur:
        ...
        with tab1:
            ...
            st.markdown(
                f"**Language:** {r_parser.language}  **Quality:** {r_parser.document_quality}"
            )
            st.markdown(
                f"**Has tables:** {'✅' if r_parser.has_tables else '❌'}  **Has numbers:** {'✅' if r_parser.has_numbers else '❌'}"
            )
        ...
        with tab3:
            st.markdown(
                f"**Risk level:** `{r_risk.risk_level.upper()}`  **Score:** {r_risk.overall_risk_score}/10  **PII:** {'⚠️ Yes' if r_risk.pii_detected else '✅ No'}"
            )
            if r_risk.pii_types:
                st.warning(f"PII types: {', '.join(r_risk.pii_types)}")
            for label, items, fn in [
                ("Compliance risks", r_risk.compliance_risks, st.error),
                ("Legal risks", r_risk.legal_risks, st.warning),
                ("Financial risks", r_risk.financial_risks, st.warning),
            ]:
                if items:
                    st.markdown(f"**{label}:**")
                    for x in items:
            ...
            if r_risk.recommendations:
                ...
                for x in r_risk.recommendations:
                    st.markdown(f"→ {x}")
        ...
        with tab4:
            ...
            if r_action.action_items:
                ...
                for a in r_action.action_items:
                    st.markdown(f"- {a}")
            if r_action.decisions_made:
                st.markdown("**Decisions:**")
                for d in r_action.decisions_made:
                    st.success(d)
            for label, items in [
                ("Deadlines", r_action.deadlines),
                ("Owners", r_action.owners),
            ]:
                if items:
                    st.markdown(f"**{label}:**")
                    for x in items:
        ...
        with tab5:
            ...
            for x in r_summary.recommendations:
                st.markdown(f"→ {x}")
        ...
        try:
            ...
            full_results = {
                ...
                "models_used": cur["models"],
                "parser": r_parser.model_dump(),
                "entities": r_entity.model_dump(),
                "risks": r_risk.model_dump(),
                "actions": r_action.model_dump(),
        ...
        st.page_link(
            ...
            label="View this run in Observability Dashboard →",
            icon="📡",
        )
...
""",
        ...
        st.page_link(
            "pages/observability.py",
            label="Open Observability Dashboard — compare both runs →",
            icon="📡",
        )
    elif pdf_base or pdf_rout:
        missing = "WITH ROUTER" if pdf_base else "WITHOUT ROUTER"
        st.info(
            f"Run the PDF pipeline **{missing}** to unlock the full dashboard comparison."
        )
