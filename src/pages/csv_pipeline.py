...
def render(visitor_fp: str):
    ...
    st.markdown(
        '<div class="section-label">Choose Pipeline Mode</div>', unsafe_allow_html=True
    )
    ...
    with col_r:
        ...
        if st.button(
            "Select: With Router (Save 70%)", use_container_width=True, key="sel_router"
        ):
...
""",
    ...
    st.page_link(
        "pages/observability.py", label="Open Observability Dashboard", icon="📡"
    )
    ...
    st.markdown(
        '<div class="section-label">Upload Your Dataset</div>', unsafe_allow_html=True
    )
    uploaded_file = st.file_uploader(
        "Drop CSV here", type=["csv"], label_visibility="collapsed"
    )
    use_demo = st.checkbox(
        "Use demo dataset — retail transactions with intentional data quality issues"
    )
    ...
    if baseline_result or routed_result:
        ...
        st.markdown(
            '<div class="section-label">Cost Dashboard</div>', unsafe_allow_html=True
        )
...
""",
        ...
        # ── Only one run: prompt to run the other ──────────────
            ...
            st.info(
                f"Run the pipeline **{missing}** to unlock the full dashboard comparison."
            )
