...
def render(visitor_fp: str):
    ...
    st.markdown(
        '<div class="section-label">Select your database</div>', unsafe_allow_html=True
    )
    ...
    st.markdown(
        f'<div class="conn-form-title">🔌 {db_type} Connection</div>',
    ...
    if db_type == "Azure Databricks":
        ...
        with col1:
            host = st.text_input(
                "Workspace URL", placeholder="adb-xxxxx.azuredatabricks.net"
            )
            http_path = st.text_input(
                "HTTP Path", placeholder="/sql/1.0/warehouses/xxxxx"
            )
        with col2:
            token = st.text_input(
                "Personal Access Token", type="password", placeholder="dapi..."
            )
            ...
            if account and user and password and database and schema and table:
                try:
                    ...
                    with st.spinner("Connecting to Snowflake..."):
                        df = fetch_table(
                            account, user, password, database, schema, table
                        )
            ...
            if host and database and user and password and table:
                try:
                    ...
                    with st.spinner("Connecting to PostgreSQL..."):
                        df = fetch_table(
                            host, int(port), database, user, password, table
                        )
                    st.success(f"Connected — {len(df)} rows fetched")
                    st.dataframe(df, use_container_width=True, height=240)
                except Exception as e:
        ...
        with col1:
            database = st.text_input(
                "Database File", placeholder="/path/to/my_database.duckdb"
            )
        with col2:
            table = st.text_input("Table", placeholder="my_table")
    ...
    if df is not None and not df.empty:
        ...
        if st.button(
            f"▶ Run Pipeline on DB Data — {db_mode_label}",
            use_container_width=True,
            type="primary",
        ):
            ...
            if not run_ok:
                st.error(
                    "No free runs remaining. Enter your GitHub username above, add your API key, or fork the repo."
                )
            else:
                ...
                guardrails = (
                    GuardrailEngine(**guardrail_cfg)
                    if guardrail_cfg
                    else GuardrailEngine()
                )
                with st.spinner(f"Running pipeline ({db_mode_label})..."):
                    result = run_pipeline(
                        tmp_path,
                        routing_enabled=db_routing_on,
                ...
                st.success(
                    f"Done — GBP {result.total_cost_gbp:.5f} · {result.total_latency_ms}ms"
                )
