...
def run(csv_preview: str, total_rows: int,
        model: str = None, span=None) -> ValidatorResult:
    if model is None:
    ...
    try:
        ...
        fallback = ValidatorResult(schema_ok=False, violations=["Could not parse response"],
                                   passed_checks=[], completeness_score=0.0)
        if span:
